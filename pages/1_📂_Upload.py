...
Mapeamento pré-configurado com base nos arquivos reais:
  - Base de Produtos (24.06.26.xlsx)
  - Saldo Consignado (SALDO CONSIGNADO.xls)
  - Faturamento
"""
...
def select_col(label, cols_list, defaults, key, required=False):
    """Selectbox com auto-seleção do default se existir nas colunas."""
...
# ═══════════════════════════════════════════════════════════════════════════════
# ARQUIVO 1 — BASE DE PRODUTOS
# ═══════════════════════════════════════════════════════════════════════════════
with st.expander("📖  **1. Base de Produtos**  (catálogo de títulos, preços, curva ABC, giro)", expanded=True):
    ...
    if f1:
        try:
            ...
            st.markdown("**Mapeamento de colunas** — pré-configurado com o modelo padrão:")
            ...
            with c2:
                ...
                p_preco    = select_col("Preço de Venda",   cols, ["Preço de Venda","Preco Venda","preco_venda"], "p_preco")
            ...
            if st.button("💾 Importar Base de Produtos", type="primary", key="btn_prod"):
                if p_isbn == "(não usar)" or p_produto == "(não usar)":
                    ...
                    for nc in ["preco_venda","estoque_sp","estoque_cl","estoque_pr","estoque_total",
                               ...
                               "vendas_6m","vendas_3m","mediana_12m","mediana_24m","custo_stand","estoque_terc"]:
...
# ═══════════════════════════════════════════════════════════════════════════════
# ARQUIVO 2 — SALDO CONSIGNADO
# ═══════════════════════════════════════════════════════════════════════════════
with st.expander("📦  **2. Saldo Consignado**  (remessa, devolução/acerto, saldo atual por cliente)", expanded=True):
    ...
    if f2:
        try:
            ...
            with c1:
                ...
                s_razao      = select_col("Razão Social",   cols, ["Cliente","Razao Social","razao_social"], "s_razao", True)

            with c2:
                st.markdown("**Livro / NF**")
                s_nf         = select_col("NF/Série",       cols, ["NF/Serie","NF","nf"], "s_nf")
                ...
                s_barras     = select_col("Cód. de Barras", cols, ["Cod Barras","Código Barras"], "s_barras")
                s_titulo     = select_col("Título",         cols, ["Titulo","titulo","Produto"], "s_titulo")
                s_autor      = select_col("Autor",          cols, ["Autor","autor"], "s_autor")
                s_desconto   = select_col("Desconto",       cols, ["Desconto","desconto"], "s_desc")
                s_status_tit = select_col("Status Título",  cols, ["STATUS","Status","status"], "s_stattit")

            with c3:
                st.markdown("**Quantidades e Valores**")
                s_qtde_rem   = select_col("Qtde Remessa",   cols, ["Qtde Remet","Qtde Remessa"], "s_qtrem", True)
                s_qtde_dev   = select_col("Qtde Dev/Acert", cols, ["Qtde Dev/Acert","Qtde Acerto"], "s_qtdev", True)
                s_qtde_saldo = select_col("Qtde Saldo",     cols, ["Qtde Saldo","Saldo"], "s_qtsaldo", True)
                s_vl         = select_col("Valor Líquido",  cols, ["Valor Liquido","Valor Liquido ","valor_liquido"], "s_vl")
                s_vb         = select_col("Valor Bruto",    cols, ["Valor Bruto","valor_bruto"], "s_vb")

            if st.button("💾 Importar Saldo Consignado", type="primary", key="btn_saldo"):
                obrig = [s_cod_cli, s_razao, s_isbn, s_qtde_rem, s_qtde_dev, s_qtde_saldo]
                if any(v == "(não usar)" for v in obrig):
                    st.error("Obrigatórios: Cód. Cliente, Razão Social, ISBN, Qtde Remessa, Qtde Dev/Acert, Qtde Saldo.")
                else:
                    mapa = {
                        "cod_loja": s_cod_loja, "cnpj": s_cnpj,
                        "codigo_cliente": s_cod_cli,
                        "loja": s_loja, "cod_gcon": s_gcon, "uf": s_uf,
                        "razao_social": s_razao, "nf_serie": s_nf,
                        "data_emissao": s_data_em, "isbn": s_isbn,
                        "cod_barras": s_barras, "titulo": s_titulo,
                        "autor": s_autor, "desconto": s_desconto,
                        "status_titulo": s_status_tit,
                        "qtde_remessa": s_qtde_rem, "qtde_dev_acert": s_qtde_dev,
                        "qtde_saldo": s_qtde_saldo,
                        "valor_liquido": s_vl, "valor_bruto": s_vb,
                    }
                    rename = {v: k for k, v in mapa.items() if v and v != "(não usar)" and v in df_raw.columns}
                    df_out = df_raw.rename(columns=rename)
...
# ═══════════════════════════════════════════════════════════════════════════════
# ARQUIVO 3 — FATURAMENTO
# ═══════════════════════════════════════════════════════════════════════════════
with st.expander("💰  **3. Faturamento dos Clientes**  (opcional — vendas realizadas)", expanded=False):

    st.info("O faturamento enriquece a análise com receita realizada mês a mês. Se ainda não tiver o arquivo, importe depois.")
    ...
    if f3:
        try:
            ...
            with c1:
                ...
                f_cod_cli  = select_col("Cód. Cliente",  cols, ["Codigo Cliente","Cod.","Cod"], "f_cc", True)
                f_loja     = select_col("Loja",           cols, ["Loja","Lj."], "f_lj")
                f_razao    = select_col("Razão Social",   cols, ["Cliente","Razao Social"], "f_razao")

            with c2:
                st.markdown("**Produto**")
                f_isbn     = select_col("ISBN / Cód. Produto",cols, ["Codigo Produto","Codigo","ISBN"], "f_isbn", True)
                f_titulo   = select_col("Título / Produto",   cols, ["Produto","Titulo"], "f_tit")
                f_editora  = select_col("Editora",            cols, ["Editora"], "f_edit")
                f_st_prod  = select_col("Status Produto",     cols, ["Status Produto"], "f_stprod")
                f_gcon     = select_col("Vendedor (Gcon)",    cols, ["Codigo Vendedor","Gcon","Vendedor"], "f_gcon")
            ...
            with c3:
                ...
                f_qtd_nao  = select_col("Qtd Não Atendida",  cols, ["Qtd Não Atendida","Qtd Nao Atendida"], "f_qtnao")
                f_controle = select_col("Controle Atendim.", cols, ["Controle Atendimento"], "f_ctrl")

            with c4:
                st.markdown("**Valores, Datas e TES**")
                f_vunit    = select_col("Valor Unitário",     cols, ["Valor Unitário","Valor Unitario"], "f_vunit")
                f_val_at   = select_col("Valor Atendido ★",  cols, ["Valor Atendido"], "f_valat", True)
                f_val_nao  = select_col("Valor Não Atendido",cols, ["Valor Não Atendido","Valor Nao Atendido"], "f_valnao")
                f_preco_c  = select_col("Preço Capa",        cols, ["Preço Capa Unitário","Preco Capa"], "f_pcapa")
            ...
            if st.button("💾 Importar Faturamento", type="primary", key="btn_fat"):
                if any(v == "(não usar)" for v in [f_cod_cli, f_isbn, f_qtd_at, f_val_at]):
                    st.error("Obrigatórios: Cód. Cliente, ISBN/Cód. Produto, Qtd Atendida, Valor Atendido.")
                else:
                    mapa = {
                    ...
                    if gcons:
                        st.info(f"Vendedores encontrados: {gcons[:8]}")
                    if f_cod_tes == "(não usar)":
                        st.warning("⚠️ Coluna TES não mapeada — sem classificação de tipo de faturamento. Importe a **Tabela TES** abaixo.")
        except Exception as e:
            st.error(f"Erro: {e}")


# ═══════════════════════════════════════════════════════════════════════════════
# ARQUIVO 4 — TABELA TES
# ═══════════════════════════════════════════════════════════════════════════════
with st.expander("🏷️  **4. Tabela TES**  (classifica o faturamento em Venda / Acerto Consignação / Envio / Outros)", expanded=False):

    st.info(
        "A Tabela TES é obrigatória para que o sistema distinga **vendas diretas** de "
        "**acertos de consignação** no faturamento. Importe o arquivo `TES COMPLETA.xlsx` "
        "ou equivalente. Deve conter pelo menos as colunas: código TES e Tipo."
    )
    ...
    if f4:
        try:
            ...
            with c1:
                ...
                t_txt      = select_col("Descrição",          cols, ["Txt Padrao","Descricao","Texto"], "t_txt")
